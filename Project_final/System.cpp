...
class System{
    ...
    public :
        ...
        int searchByTimeCrime(int year, int month, int day, int hour, int minute, string type, char** array, int row, int col);
        int searchByIntervalTimeCrime(int year1, int month1, int day1, int hour1, int minute1, int year2, int month2, int day2, int hour2, int minute2, string type, char** array, int row, int col);
...
void System::Main(){
    ...
    do{
        if(choice==1){
            ...
            searchByTime(time[0], time[1], time[2], time[3], time[4], NULL, 0, 0);
        }
        else if(choice==4){
            ...
            searchByIntervalTime(time1[0], time1[1], time1[2], time1[3], time1[4], time2[0], time2[1], time2[2], time2[3], time2[4], NULL, 0, 0);
        }
        else if(choice==5){
            string type;
            ...
            searchByTimeCrime(time[0], time[1], time[2], time[3], time[4], type, NULL, 0, 0);
        }
        else if(choice==6){
            string type;
            cout << "Input the type of the crime\n";
            ...
            searchByIntervalTimeCrime(time1[0], time1[1], time1[2], time1[3], time1[4], time2[0], time2[1], time2[2], time2[3], time2[4], type, NULL, 0, 0);
        }
        else if(choice==7){
            string name;
            cout << "Input the name of the criminal\n";
...
//  搜尋某個特定時間點的犯罪紀錄
int System::searchByTime(int year, int month, int day, int hour, int minute, char** array, int row, int col){
    bool f = false;
    int index = 0;  //  current row in 2D array
    for(int i=0; i<5; i++){
        vector<CrimeRecord*> t = crimeTree[i].SearchTime(year, month, day, hour, minute);
        for(int i=0;i<t.size();i++){
            f=true;
            cout << "-------------------------------------\n";
            t[i]->getCriminal()->outputAllinfo(array, row, col, index);
            cout << *t[i];
            cout << "-------------------------------------\n";

            //  Copy the information of crime to the 2D array from CrimeRecord
            strcpy(array[index], t[i]->getType().c_str()); index++;
            strcpy(array[index], t[i]->getTime_str().c_str()); index++;
            strcpy(array[index], t[i]->getDescription().c_str()); index++;
        }
    }
    if(!f){
        cout << "No crime on that time\n";
        strcpy(array[index], "No crime on that time");
        return 1;
    }
    return index / 8;
}
...
//  搜尋某個時間區間的犯罪紀錄
int System::searchByIntervalTime(int year1, int month1, int day1, int hour1, int minute1, int year2, int month2, int day2, int hour2, int minute2, char** array, int row, int col){
    bool f = false;
    int index = 0;  //  current row in 2D array
    for(int i=0; i<5; i++){
...
//  搜尋某個特定時間點的特定形式犯罪紀錄
int System::searchByTimeCrime(int year, int month, int day, int hour, int minute, string type, char** array, int row, int col){
    bool f = false;
    int index = 0;  //  current row in 2D array
...
//  搜尋某個時間區間的特定形式犯罪紀錄
int System::searchByIntervalTimeCrime(int year1, int month1, int day1, int hour1, int minute1, int year2, int month2, int day2, int hour2, int minute2, string type, char** array, int row, int col){
    bool f = false;
    int index = 0;  //  current row in 2D array
...
extern "C" {
    __declspec(dllexport) void newCriminal(char* criminalType, char* description, int year, int month, int day, int hour, int minute, char* id, char* bir, char* gen, char* loc, char* name, char** array, int row, int col){
...
extern "C" {
    __declspec(dllexport) void deleteCriminal(char* name, int year, int month, int day, int hour, int minute, char** array, int row, int col){
        vector<int> time(5, 0);
        time[0] = year;
        time[1] = month;
        time[2] = day;
...
extern "C" {
    __declspec(dllexport) int searchbyTime(int year, int month, int day, int hour, int minute, char** array, int row, int col){
        return s.searchByTime(year, month, day, hour, minute, array, row, col);
    }
}

extern "C"{
    __declspec(dllexport) int searchbyIntervalTime(int year1, int month1, int day1, int hour1, int minute1, int year2, int month2, int day2, int hour2, int minute2, char** array, int row, int col){
        return s.searchByIntervalTime(year1, month1, day1, hour1, minute1, year2, month2, day2, hour2, minute2, array, row, col);
    }
}

extern "C"{
    __declspec(dllexport) int searchbyTimeCrime(int year, int month, int day, int hour, int minute, char* type, char** array, int row, int col){
        return s.searchByTimeCrime(year, month, day, hour, minute, type, array, row, col);
    }
}

extern "C"{
    __declspec(dllexport) int searchbyIntervalTimeCrime(int year1, int month1, int day1, int hour1, int minute1, int year2, int month2, int day2, int hour2, int minute2, char* type, char** array, int row, int col){
        return s.searchByIntervalTimeCrime(year1, month1, day1, hour1, minute1, year2, month2, day2, hour2, minute2, type, array, row, col);
    }
}
...
extern "C" {
    __declspec(dllexport) void test(char** array, int row, int col) {
        char star[] = {'*', '*', '*', '\0'};
