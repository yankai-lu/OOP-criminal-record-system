...
class AddPage(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.ensure = Button(self.rect.right - 200 - 50, self.rect.bottom - 50 - 50, 200, 50, "ADD", RED, None, 30, WHITE)
    ...
    def handle_mouse_hover(self, mouse_pos) -> None:
        if(self.is_crime_open):
    ...
    # check whether all the input box is filled
    # return if all the input box is filled or not, and the string of the waring input box
    def isComplete(self) -> tuple[bool, str]: 
        for input_box in self.inputBoxGroup:
            if  input_box.text == input_box.original_text or input_box.text == '':
                return True, input_box.original_text
        if self.crime.text == "(Crime Type)": 
            return True, "Type"
    ...
    def stringTransfer(self) -> str:
        if not self.isComplete():
            SystemError("The input box is not complete")
        
        if self.crime.text == "Theft":
            return "steal"
        elif self.crime.text == "Robbery":
            return "rob"
        elif self.crime.text == "Fraud":
            return "scam"
        elif self.crime.text == "Murder":
            return "kill"
        else:
            return "others"