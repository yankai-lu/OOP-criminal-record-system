from function import pygame, WINDOW_WIDTH, WINDOW_HEIGHT, WHITE, BLACK, LIGHT_BLUE, BLUE, RED, PURPLE, LIGHT_PURPLE, GRAY, LIGHT_RED
...
class SearchPage(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.ensure = Button(self.rect.right - 200 - 50, self.rect.bottom - 50 - 50, 200, 50, "SEARCH", RED, None, 30, WHITE)
        self.searchByName =             Button(self.x + 100, self.y + 300, 300, 50, "Search by Name", BLUE, "font\\calibri-regular.ttf", 20, WHITE)
        self.searchByTime =             Button(self.x + 100, self.y + 400, 300, 50, "Search by Time", BLUE, "font\\calibri-regular.ttf", 20, WHITE)
        self.searchByTimeCrime =        Button(self.x + 100, self.y + 500, 300, 50, "Search by Time and Crime", BLUE, "font\\calibri-regular.ttf", 20, WHITE)
        self.searchByIntervalTime =     Button(self.x + 600, self.y + 400, 300, 50, "Search by Interval Time", BLUE, "font\\calibri-regular.ttf", 20, WHITE)
        self.searchByIntervalTimeCrime =Button(self.x + 600, self.y + 500, 300, 50, "Search by Interval Time and Crime", BLUE, "font\\calibri-regular.ttf", 20, WHITE)
        self.buttonsGroup = pygame.sprite.Group()
        self.buttonsGroup.add(self.searchByName, self.searchByTime, self.searchByTimeCrime, self.searchByIntervalTime, self.searchByIntervalTimeCrime)
    ...
    def draw(self, surface):
        ...
        #self.ensure.draw(surface) # not used now
        ...
        if(self.is_crime_open):
            ...
            for button in self.buttonsGroup:
                if button.rect.collidepoint(mouse_pos):
                    button.set_color(LIGHT_BLUE)
                    button.set_text_color(BLACK)
                else:
                    button.set_color(BLUE)
                    button.set_text_color(WHITE)
