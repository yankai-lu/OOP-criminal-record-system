...
namespace CriminalSystem{    
    ...
    CrimeRecord::CrimeRecord(string t,string d,int y,int month,int day,int hour,int minute,Criminal* crimi,int c,CrimeRecord *l,CrimeRecord *r,CrimeRecord *parent):type(t),description(d),left(l),right(r),p(parent),color(c),criminal(crimi){
        this->setTime(y, month, day, hour, minute);
        /*
    ...
    string CrimeRecord::getTime_str() const
    {
        return time_str;
    }
    void CrimeRecord::setTime(int y, int month, int day, int hour, int minute){
        ...
        //  store the time in string format
        int index = 0;  //  character index of the string
        for(int value = 1000; value > 0; value /= 10)
        {
            time_str[index] = y / value + '0';
            y %= value;
            index++;
        }
        time_str[index] = ' ';  index++;

        for(int value = 10; value > 0; value /= 10)
        {
            time_str[index] = month / value + '0';
            month %= value;
            index++;
        }
        time_str[index] = '/';  index++;
    ...
    string CrimeRecord::getDescription()
    {
        return this->description;
    }
    void CrimeRecord::setDescription(string d)
    {
        this->description = d;
    }
